...
- Spawn 100 users over 10 seconds (10 users/second spawn rate)
- Maintain 10        response = session.post(url=host+"/api/v1/users/login", headers=headers, json=body, timeout=30) RPS for 15 minutes
- Save latency data per route to CSV file
- Accept configurable URL via LOCUST_BASE_URL environment variable

Usage:
  # Run with web UI (manual start):
  locust -f locustfile.py --host=http://localhost:8080
  
  # Run headless (automatic start and stop):
  locust -f locustfile.py --headless --host=http://localhost:8080
  
  # With custom URL and skipping user creation (recommended for CI/CD):
  SKIP_USER_CREATION=1 LOCUST_BASE_URL=http://localhost:8080 locust -f locustfile.py --headless --host=$LOCUST_BASE_URL
  
  # The test will automatically exit after 15 minutes when using OptimizedLoadShape
"""
...
# Set test duration for 15 minutes (900 seconds)
...
# Create CSV file immediately so it exists even if test fails early
csv_file = open('latency_data.csv', 'w', newline='')
csv_writer = csv.writer(csv_file)
csv_writer.writerow(['timestamp', 'response_time_ms', 'request_type', 'name', 'response_length', 'exception'])
...
def on_spawning_complete(user_count, **kwargs):
    global spawning_complete, full_capacity_reached, csv_file, csv_writer
    spawning_complete = True
    
    # Mark full capacity when we reach 100 users
    if user_count >= 100:
        full_capacity_reached = True
        logging.info(f"🎯 Full capacity reached! All {user_count} users spawned.")
        logging.info(f"📊 Now starting CSV logging for steady-state performance data...")
        
        # CSV file already created at startup, just mark that we can start logging data

@events.request.add_listener
def on_request(request_type, name, response_time, response_length, exception, context, **kwargs):
    # Only log to CSV after full capacity (100 users) is reached
    if full_capacity_reached and csv_writer:
        csv_writer.writerow([
            time.time(),
            response_time,
            request_type,
...
def get_json_from_response(response):
    if response.content is None or len(response.content) == 0:
        raise Exception(f"Empty response content. Status code: {response.status_code}, URL: {response.url}")
    try:
        response_as_text = response.content.decode('UTF-8')
        response_as_json = json.loads(response_as_text)
        return response_as_json
    except json.JSONDecodeError as e:
        raise Exception(f"Failed to parse JSON response. Status code: {response.status_code}, Content: {response_as_text[:200]}...")
    except Exception as e:
        raise Exception(f"Error processing response. Status code: {response.status_code}, Error: {str(e)}")
...
def create_users(host):
    USER_POOL.clear()
    host = "http://192.5.87.25:30001"
    def api_call_admin_login():
        headers = {"Accept": "application/json", "Content-Type": "application/json"}
        body = {"username": "admin", "password": "222222"}
        response = requests.post(url=host+"/api/v1/users/login", headers=headers, json=body)
    ...
    USER_POOL.append(("admin", admin_token))

@events.init.add_listener
def on_locust_init(environment, **kwargs):
    global BASE_URL
    # Get base URL from environment variable or use the host from environment
    BASE_URL = os.getenv('LOCUST_BASE_URL') or environment.host or 'http://192.5.87.25:30001'
    
    # Skip user creation if environment variable is set
    if not os.getenv('SKIP_USER_CREATION'):
        logging.info(f"Creating users with base URL: {BASE_URL}")
        create_users(BASE_URL)
    else:
        logging.info("Skipping user creation (SKIP_USER_CREATION=1)")
        # Add a default admin user to USER_POOL
        USER_POOL.append(("admin", "222222"))

@events.quitting.add_listener
def on_locust_quit(environment, **kwargs):
    """Close request log file when locust quits"""
    request_log_file.close()
    if csv_file:
        csv_file.close()
        logging.info("CSV file closed")
    logging.info("Request log file closed")

@events.test_stop.add_listener
def on_test_stop(environment, **kwargs):
    """Handle test stop event and trigger quit"""
    logging.info("Test stopped - initiating shutdown")
...
def login(client, username, password):
    def api_call_login():
        ...
        response = client.post(url="/api/v1/users/login", headers=headers, json=body, name=get_name_suffix("login"), timeout=30,
            context={'type': 'login'})
        response_as_json = get_json_from_response(response)
        return response_as_json, response_as_json["status"]
...
def get_departure_date():
    # Only use today and tomorrow for departure dates
    today = datetime.now().date()
    tomorrow = today + timedelta(days=1)
    departure_date = random.choice([today, tomorrow])
...
def get_trip_information(client, from_station, to_station):
    ...
    def api_call_get_trip_information():
        ...
        logging.info(f"Making travel request to: {getattr(client, 'base_url', 'NO_BASE_URL')}/api/v1/travelservice/trips/left")
        logging.info(f"Request body: {body}")
        
        response = client.post(url="/api/v1/travelservice/trips/left", headers=headers, json=body, 
                    name=get_name_suffix("get_trip_information"),
                    context={'type': 'get_trip_information'})
        
        logging.info(f"Travel response status: {response.status_code}, URL: {getattr(response, 'url', 'NO_URL')}")
        response_as_json = get_json_from_response(response)
        return response_as_json, response_as_json["status"]

    try:
        return try_until_success(api_call_get_trip_information)
    except Exception as e:
        logging.warning(f"Travel service failed completely: {e}")
        # Return a dummy successful response to continue the test
...
def book(client, user_id, username):
    ...
    def api_call_insurance():
        response = client.get(url="/api/v1/assuranceservice/assurances/types", name=get_name_suffix("get_assurance_types"),
            context={'type': 'insurance'}, timeout=20)
        response_as_json = get_json_from_response(response)
        return response_as_json, response_as_json["status"]
    ...
    def api_call_contacts():
        response = client.get(url=f"/api/v1/contactservice/contacts/account/{user_id}", name=get_name_suffix("query_contacts"),
            context={'type': 'contacts'}, timeout=20)
        response_as_json = get_json_from_response(response)
        return response_as_json, response_as_json["status"]
    ...
    def api_call_ticket():
        ...
        response = client.post(url="/api/v1/preserveservice/preserve", json=body, name=get_name_suffix("preserve_ticket"), timeout=20,
            context={'type': 'ticket'})
        response_as_json = get_json_from_response(response)
        return response_as_json, response_as_json["status"]
...
class UserBooking(HttpUser):
    # Set wait time to achieve 100 RPS with 100 users (1 request per user per second)
    wait_time = between(0.9, 1.1)  # Wait ~1 second between tasks for each user
    
    # Explicitly set the host (will be overridden by --host parameter)
    host = "http://localhost:30001"

    def on_start(self, sleep=True):
        # Use fresh user and token from USER_POOL, skip login
        self.username, self.token = USER_POOL[0]
        if sleep:
            time.sleep(random.expovariate(lambd=1/INTERVAL))
        self.client.headers.update({"Authorization": f"Bearer {self.token}"})
        self.client.headers.update({"Content-Type": "application/json"})
        self.client.headers.update({"Accept": "application/json"})
        self.user_id = self.username  # Set user_id to username for reference
    ...
    def perform_task(self):
        ...
        try:
            ...
            logging.warning(f"Travel service failed, continuing with other operations: {e}")
        
        if random.randrange(10) == 0:
            try:
                book(self.client, self.user_id, self.username)
            except Exception as e:
                logging.warning(f"Booking failed: {e}")

class CustomShape(LoadTestShape):
    """
    Spawn 100 users for 10 minutes (600 seconds)
    Total test time: 10 seconds (ramp-up) + 600 seconds (steady-state) = 610 seconds
    """
    ...
    def tick(self):
        ...
        if run_time < 10:
            ...
            return (100, 10)
        elif run_time < 610:  # 10 seconds ramp-up + 600 seconds (10 minutes) test
            # Hold 100 users for 10 minutes
            return (100, 10)
        else:
            # Test completed - stop the test
            logging.info(f"Test completed after {run_time} seconds (10 minutes). Stopping...")
            return None