#!/usr/bin/env pythondef find_csv_files(model_name='ssd'):
    ...
    for file_path in csv_files:
        ...
        if model_name.lower() not in filename.lower():
            continueots for SSD throughput data across different configurations.
...
def find_resnet_csv_files():
    """Find all ResNet CSV files and organize by type and configuration."""
    base_dir = "/home/cc/Results/sep23/2024-biosys-ec-elasticcontainer/Machine-learning-models"
    ...
    # Search pattern for ResNet CSV files
    pattern = f"{base_dir}/resnet/exported-csv/*/*.csv"
    csv_files = glob.glob(pattern)
    ...
    for file_path in csv_files:
        ...
        # Only process ResNet files
        if 'resnet' not in filename.lower():
            continue
...
def load_throughput_data(files_data):
    """Load throughput data from CSV files and organize for box plots."""
    throughput_data = {
        'training': {},
        'inference': {}
    }
    ...
    for operation in ['training', 'inference']:
        for config, file_path in files_data[operation].items():
            try:
                ...
                # For ResNet, inference throughput column is 'inference_throughput_examples_per_sec'
...
def create_box_plots(throughput_data):
    ...
    for operation in ['training', 'inference']:
        ...
        # Minimal labels
        plt.title(f'ResNet {operation.title()}')
        plt.ylabel('Throughput (examples/sec)')
        
        plt.tight_layout()
        
        # Save the plot
        output_file = f'resnet_{operation}_boxplot.png'
...
def print_statistics_summary(throughput_data):
    ...
    for operation in ['training', 'inference']:
        ...
        print(f"\n{operation.upper()} THROUGHPUT STATISTICS (examples/sec)")
        print("=" * 60)
...
def main():
    """Main function to create ResNet throughput box plots."""
    print("Finding ResNet CSV files...")
    files_data = find_resnet_csv_files()
