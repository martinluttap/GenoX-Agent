...
parser = argparse.ArgumentParser(
    prog="autothrottle",
    description=f"Experiments for autothrottle.",
...
def run_exp_cleanup(exp_dir: str = '') -> None:
    ...
    # Remove Nextflow's post-execution artefacts
    run_cmd(f'sudo rm -rf {TOP_DIR}/work')
    run_cmd(f'sudo rm -rf {TOP_DIR}/.nextflow*')
...
if __name__ == "__main__":
    ...
    try:
        ...
        WORKFLOW=f"{AGENT_DIR}/experiments/nf_scripts/{args.app}.nf"
        INPUT_CONFIG=f"{AGENT_DIR}/experiments/configs/{args.app}.config"
        PATH_CONTROLLER=f"{AGENT_DIR}/controller/controller.go"
        APP=args.app
